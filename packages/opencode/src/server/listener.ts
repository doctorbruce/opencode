...
type Input<E> = Options & {
  routes: Layer.Layer<never, E, RouteRequirements>
...
export async function listen<E>(input: Input<E>): Promise<Listener> {
  const listener = await Effect.runPromise(listenEffect(input))
  return {
...
const listenEffect = Effect.fn("ServerListener.listen")(function* <E>(input: Input<E>) {
  const state = yield* startWithPortFallback(input)
  const address = yield* tcpAddress(state)
  const listenerUrl = makeURL(input.hostname, address.port)
  const unpublishMdns = yield* setupMdns(input, address.port, state.scope)
  ServerAddress.set(listenerUrl)
  ...
  } satisfies EffectListener
})

function listenerLayer<E>(input: Input<E>, port: number) {
  return HttpRouter.serve(input.routes, {
    middleware: disposeMiddleware,
    disableLogger: true,
    disableListenLog: true,
  }).pipe(
    Layer.provideMerge(WebSocketTracker.layer),
    Layer.provideMerge(serverLayer({ port, hostname: input.hostname })),
    // Install a fresh `ConfigProvider` per listener so `Config.string(...)`
    // reads reflect the current `process.env`. Effect's default
    // `ConfigProvider` snapshots `process.env` on first read and caches the
    // result on a module-singleton Reference; without overriding it here,
    // every later listener keeps observing that initial snapshot.
    Layer.provide(ConfigProvider.layer(ConfigProvider.fromEnv())),
  )
}

function startWithPortFallback<E>(input: Input<E>) {
  if (input.port !== 0) return startListener(input, input.port)
  // Match the legacy listener port-resolution behavior: explicit `0` prefers
  // 4096 first, then any free port.
  return startListener(input, 4096).pipe(Effect.catch(() => startListener(input, 0)))
}

function startListener<E>(input: Input<E>, port: number) {
  const scope = Scope.makeUnsafe()
  return Layer.buildWithMemoMap(listenerLayer(input, port), Layer.makeMemoMapUnsafe(), scope).pipe(
...
function setupMdns(opts: Options, port: number, scope: Scope.Scope) {
  return Effect.gen(function* () {
    const publish =
  ...
  return Effect.gen(function* () {
    ...
    const closeScopeOnce = yield* Effect.cached(
      Scope.close(state.scope, Exit.void).pipe(
        Effect.ignore,
        Effect.ensuring(Effect.sync(() => ServerAddress.clear(listenerUrl))),
      ),
    )
...
export * as ServerListener from "./listener"
