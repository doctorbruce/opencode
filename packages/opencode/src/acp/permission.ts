...
async function permissionToolCall(input: {
  ...
  return {
    ...toolCall,
    title: displayString(input.display?.title) ?? toolCall.title,
    rawInput: input.display?.rawInput ?? toolCall.rawInput,
    locations: displayLocations(input.display?.locations) ?? permissionLocations(input.toolName, input.input),
