...
type CloseSignal = Deferred.Deferred<readonly [code: number | null, signal: NodeJS.Signals | null]>
      ...
      switch (command._tag) {
        case "StandardCommand": {
          ...
          const [proc, exited, closed] = yield* Effect.acquireRelease(
            ...
            Effect.fnUntraced(function* ([proc, exited]) {
          ...
          return makeHandle({
            ...
            exitCode: Effect.flatMap(Deferred.await(closed), ([code, signal]) => {
